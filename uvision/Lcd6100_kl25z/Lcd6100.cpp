...
void Lcd6100::LCDCommand(unsigned char data){
	...
	cbi(LCD_PORT_CS, LCD_CS_POS);     // enable chip
	cbi(LCD_PORT_DIO, LCD_DIO_POS);   // output low on data out (9th bit low = command)

	cbi(LCD_PORT_SCK, LCD_SCK_POS);   // send clock pulse
	delayMicroseconds(1);
	sbi(LCD_PORT_SCK, LCD_SCK_POS);

	for (jj = 0; jj < 8; jj++)
	{
		if ((data & 0x80) == 0x80)
			sbi(LCD_PORT_DIO, LCD_DIO_POS);
		else
			cbi(LCD_PORT_DIO, LCD_DIO_POS);
	...
	sbi(LCD_PORT_CS, LCD_CS_POS);     // disable
}
...
void Lcd6100::LCDData(unsigned char data)
{
	...
	cbi(LCD_PORT_CS, LCD_CS_POS);     // enable chip
	sbi(LCD_PORT_DIO, LCD_DIO_POS);   // output high on data out (9th bit high = data)

	cbi(LCD_PORT_SCK, LCD_SCK_POS);   // send clock pulse
	delayMicroseconds(1);
	sbi(LCD_PORT_SCK, LCD_SCK_POS);   // send clock pulse
	...
	LCD_PORT_CS	|=	(1<<LCD_CS_POS);  // disable
}

void Lcd6100::init(){
	Lcd6100::init(PHILLIPS);
}
...
void Lcd6100::init(int type){
	...
	cbi(LCD_PORT_SCK, LCD_SCK_POS);     //CLK = LOW
	cbi(LCD_PORT_DIO, LCD_DIO_POS);     //DIO = LOW
	delayMicroseconds(10);
	sbi(LCD_PORT_CS, LCD_CS_POS);       //CS = HIGH
	delayMicroseconds(10);
	cbi(LCD_PORT_RES, LCD_RES_POS); //RESET = LOW
	delay(100);
	sbi(LCD_PORT_RES, LCD_RES_POS); //RESET = HIGH
	delay(100);
	sbi(LCD_PORT_SCK, LCD_SCK_POS);     // SCK = HIGH
	sbi(LCD_PORT_DIO, LCD_DIO_POS);     // DIO = HIGH
	delayMicroseconds(10);
