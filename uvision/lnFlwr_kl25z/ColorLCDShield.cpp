...
LCDShield::LCDShield()
{
	...
	PDDR(B) = ((1<<CS)|(1<<DIO)|(1<<SCK)|(1<<LCD_RES));  //Set the control pins as outputs
...
void LCDShield::init(int type)
{
	...
	// added by me
	LCDCommand(INVOFF);	// set display inversion ON
...
void LCDShield::setChar(char c, int x, int y, int fColor, int bColor)
{
	...
	int             i,j;
	unsigned int    fontColsPerChar;
	unsigned int    fontRowsPerChar;
	...
	// get the nColumns, fontRowsPerChar and numberOfBytes
	fontColsPerChar = *pFont;
	fontRowsPerChar = *(pFont + 1);
	...
	if (driver)	// if it's an epson
	{
		...
		LCDCommand(PASET);
		LCDData(x);
		LCDData(x + fontRowsPerChar - 1);
		// Column address set (command 0x2A)
		LCDCommand(CASET);
		LCDData(y);
		LCDData(y + fontColsPerChar - 1);
	
		// WRITE MEMORY
		LCDCommand(RAMWR);
		// loop on each row, working backwards from the bottom to the top
		for (i = fontRowsPerChar - 1; i >= 0; i--) {
			// copy pixel row from font table and then decrement row
			PixelRow = *pChar++;
			// loop on each pixel in the row (left to right)
			// Note: we do two pixels each loop
			Mask = 0x80;
			for (j = 0; j < fontColsPerChar; j += 2) 
			{
	...
	//	pChar += numberOfBytes - 1;  // stick pChar at the end of the row - gonna reverse print on phillips
		pChar += 0;  // stick pChar at the end of the row - gonna reverse print on phillips
	//	for (i = fontRowsPerChar - 1; i >= 0; i--) {
		for (i = 0; i < fontRowsPerChar; i++) {
			// copy pixel row from font table and then decrement row
		//	PixelRow = *pChar--;
			PixelRow = *pChar++;
			// loop on each pixel in the row (left to right)
		...
		//	for (j = 0; j < fontColsPerChar; j += 2) 
			for (j = fontColsPerChar - 1; j >= 0; j -= 2) 
			{
				// if pixel bit set, use foreground color; else use the background color
				// now get the pixel color for two successive pixels
				if ((PixelRow & Mask) == 0)
